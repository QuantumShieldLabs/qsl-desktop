...
**RETROACTIVE RECORD — written 2026-09-04 by NA-0778 (`KICKOFF_invitations_lane_20260903.md` L7; R3 of the operator's 09-02 bank), from PR #50's own content; the stub above is untouched, and nothing above it is rewritten.**

- **Subject:** qsl-desktop PR #50 `NA-0774 (D-0045): the contacts list scrolls, the tick is quiet, the identity pane tells the truth` — branch `na0774-ui-fixes`, head `197b872c`, base `main`, merged **2026-08-31T18:14:26Z** as merge commit **`b6d9237a`** by the operator; the records PR was qsl-protocol #1811 (spine `D-1417`), merged first. Goals G4.
- **What it shipped (three things every user sees), as the PR states them:** (a) the contacts list scrolls inside its pane — measured before the change (30 rows in a 673px window: `#contacts-rows` 1410px with `scrollHeight == clientHeight`, `.list-pane` 1496px), and cured with TWO rules because the single rule the brief asked for was applied and measured to leave the pane at 1496px; after: rows client 556, pane 642 ≤ 673, `scrollHeight > clientHeight`. (b) the busy indicator is quiet under a tick — a depth-counted flag raised around `relayScanOnce`, not `relayScan`, because a scan's rerun can carry a different source than the pass that started it; with the stated bound that a user-sourced call made during one of the pass's awaits is also suppressed. (c) the identity pane stops lying (`E5`) — `refreshIdentityPane` no longer swallows a thrown `identity_show` into the "No identity exists yet" branch; `#identity-read-error` names RETRY; the error path is not drivable end-to-end because `window.__TAURI__.core` is frozen, so it is sealed by unit seals.
- **Files (8):** `ui/index.html`, `ui/main.js`, `ui/style.css`, `src-tauri/tests/gui_driver.rs`, `src-tauri/tests/harness/scenarios/f_o_contacts_scroll.json`, `src-tauri/tests/harness/scenarios/f_p_tick_quiet_busy.json`, `src-tauri/tests/na0774_identity_error_state.rs`, `scripts/ci/EXPECTED_TEST_INVENTORY.txt`. Driver flows 14 → 16.
- **Red arms, as printed in the PR:** (a) rc=101 `NO-SCROLL scroll=1410 client=1410`; (b) rc=101 `UNHID 2 time(s) under a tick`, with the positive-control row PASS (`user-call-shows-indicator`); (c) rc=101, all three unit seals fail.
- **Bounds, as stated:** zero `src-tauri/src/**` bytes; zero Tauri commands added, renamed or changed; the suite at merge 195 passed + 17 ignored = 212 = the pin.
- ⚠ **Claim boundary of THIS record:** written from the PR's body and file list as read at 0b87209b, not from a re-flight; every figure above is the PR author's, cited as such.
...
## D-0047 — NA-0778: THE INVITATIONS PAGE. Settings > Invitations from the record it can actually read, the Contacts pane's three-link block, and the show-once creation dialog.

- **Date/Lane:** 2026-09-04 · NA-0778 (`D-1421`) · desktop, display-only · Goals G4
- **Ordered by:** `KICKOFF_invitations_lane_20260903.md` L1/L2/L7 and `RULING_NA0778_001`…`004` (R21–R28), on mockups 16 (blessed 2026-08-31) and 17 (blessed 2026-09-01) as LAYOUT authority with colour from the shipped tokens; the operator's two words at STOP 003: L4 is OUT of this lane (to the label-split lane), and a redeemed row reads "Accepted".

**WHAT SHIPPED.** (1) `Settings > Invitations` — a new pane (`#pane-invitations`, its rail entry after Vault and Security) rendering the Sent rows from `invite_list`: **Waiting for reply** (active, unexpired; Revoke when the record is revocable), **Expired** (the facade's read-time overlay, or active past expiry by this clock; NO action — deviation ii), **Accepted** (redeemed; no action), and the shipped **Didn't finish** (a `creating` record; Clear through `invite_clear`, the one state that verb accepts); the date is the MINT time labelled `Sent <date>`; the count line and the All/Waiting/Accepted/Expired filters; the mockup's empty state verbatim with "Create one from Contacts" as a link; the Received group WITH ITS EXPLANATION (the engine keeps no readable record of redeemed codes — filed, protocol-side); the nudge line built from the CONTACTS list ("N connected contacts aren't verified yet…", a Verify link per name landing on the contact's detail — the shipped verification surface). (2) The Contacts pane's block per mockup 17: a NON-CLICKABLE "Invitations" label (`--fg`, never blue) and three plain text links — review → Settings > Invitations; redeem → the code-entry view, through the chooser's own opener so its finish scan still fires; send → the mint through `openInviteModal` — separated by muted dots at the shipped 6px token, no count badge; the empty state on two lines. (3) The creation dialog: "This is the only time you'll see the code…" with the lost-code guidance, PRESENT FROM OPEN because NA-0766 item 6 forbids anything appearing on activation; and the close confirmation "Did you share the code?" asked of the user's three gestures (Close, Escape, the scrim) only after a mint, through `inviteRequestClose`; `show()` still closes unconditionally.

**RULING 004 R22, BUILT IN:** the pane renders its rows only after `invite_list` and the contacts refresh have both returned (a loading line until then); it writes back into no editable field (it has none); every row action carries the invitation id in its own dataset and the handler reads that, never a row position. R21's scenario cure rides in `f_d` as DATA: the settle wait on the loaded value (`AUTOLOCK_DEFAULT_MINUTES`), the discriminator before the click, the post-click status read.

**DEVIATIONS FROM THE BLESSED MOCKUP, STATED, NOT ABSORBED:** (i) no "Connected · not verified" / "Connected · verified" rows and no "Connected <date>": the record links an invitation to no contact and carries no connected-at (R23; filed, protocol-side); (ii) no "Clear" on an expired row: `invite_clear` refuses every state but `creating` (invite/mod.rs:985-999) — list hygiene for dead records is the engine-hygiene lane's; (iii) the lede's "never sent anywhere" clause is not shipped — the typed name lands in the session filenames and the AAD until the label split (RULING 004 R25's reading); the sentence reads "stored only on this device", which is measured true; (iv) the "Verify" pop-up the kickoff names does not exist in this build; the Verify links land on the contact detail.

⚠ **CORRECTION at 004a (`RULING_NA0778_005` R30, D-5 against the Director):** deviation (iii) is WITHDRAWN. The clause is TRUE today — the L5 census measured the wire at zero for the contact label by two independent sweeps; three filenames and an AAD are on the device, which is what "stored on this device" says — so the blessed lede is restored verbatim from mockup 16 :143 in the file's own dash idiom. The trim above stands as the record of what shipped at b67b95db and why it was flagged.

⚠ **004a, THE SECOND COMMIT (`RULING_NA0778_006` R37–R43, on the SR-15 cold read `FINDINGS_SR15_NA0778_read_20260904T144353Z`; the operator's words: R42 FIX, R31 SHOWN, R33 BLESS).** Nothing above is rewritten; each fix is noted here beside the record it amends.
- **R38 (F-01, MEDIUM):** a successful Revoke now moves the in-memory row set as well as the DOM — `invitationsMarkRevoked(id)`, one function the handler's success branch calls: the record marked revoked, the count line re-rendered from the updated set, the row dropped on the next render. Before it, a chip click resurrected the revoked row.
- **R39 (F-02 MEDIUM, F-03 LOW):** paint first, then scan, for both rail links — `redeem` opens the code-entry view through its own opener (`openRedeemEntry`, the view painted before the surface-open scan is awaited); `send` paints the mint and then fires the same surface-open trigger "+" fires. The chooser opener is untouched (its literal is pinned).
- **R40 (F-04, LOW):** `openInviteModal` guards a live mint — a re-open while a code is minted routes to the one close request (the question), never to a fresh entry; a fresh open hides any question. The missing focus trap is pre-existing and product-wide: filed as a small item at the close.
- **R41:** F-07 the no-badge assertion discriminates (a slice of the block, non-vacuous); F-09 the negative arm's sixth exact count (`mint.then(` == 1) names the clipboard sink; F-12 the `invite_list` read failure uses the shared error vocabulary (a "list" arm); F-15a a chip with no matching rows shows "No invitations match this filter." (the seat's copy, a third sentence, stated); F-16 f_n's stale note corrected; F-20 the cites name the desktop's pinned qsc rev **63ece4fe** (byte-identical to f32a4c20 for the invite verbs); F-21 the Sent table's header word is **"Sent"**; F-22 every anchor-as-button in the pane answers Enter/Space through one binder.
- **R42 (F-14, E-3 of the read — pre-existing at the base, FIXED here on the operator's word):** `contactUiState` compared a contact record's state to the upstream UPPERCASE `"CHANGED"` while the gateway emits the facade's `"changed"`; the peer-key-changed arm — the MITM tell, "Verification code (new)", the code-first layout — was unreachable from live data since NA-0764, and the desktop's own pins and f_n's planted rows carried the uppercase, so every arm was green. The literal is the wire form; the precedence pin and f_n's plants follow it; the census of every comparison of a contact record's `state` in `ui/main.js` rides STOP 004a (two sites, zero unclassified). Its red arm: the uppercase restored in a scratch copy, f_n red, restored.
- **R31 (the operator: SHOWN):** expired rows shown with no action, as built, until an engine verb exists (ENG-D). **R33 (BLESS):** the Received explanation and the confirmation's second sentence stay as written.
- **R30:** the lede is the blessed sentence, verbatim (the correction above).
- **004c (`RULING_NA0778_008` on the operator's flight of 4c7b0c76, `RBANK_flight_rulings_NA0778_20260904`): THE OPERATOR'S DESIGN RULINGS, SUPERSEDING the mockups and the 08-31 bank where they differ (mark, not rewrite: the banks stay).** R52 the Contacts pane's two-line empty state sits directly below the three links. R53 the Invitations page has no page-level explanation of codes and no page-level link: the Sent empty state is "You haven't sent any invitations yet."; each group carries its OWN link only while it has nothing to list — "Sent — send invitation" (the mint) and "Received — redeem invitation" (the code-entry view; shown today, since Received has no source until ENG-B). R54 the creation dialog's show-once line is exactly "Warning: this is the only time you'll see the code.", on the left of the copy link's row, present from open; THERE IS NO CLOSE CONFIRMATION — after a mint, Close, Escape and the scrim simply close (the Director's note: an accidental close loses an unshared code, and the recovery is no longer explained anywhere; the operator flew it and ruled); the behind-the-scrim guard (R40) is a NO-OP that leaves the open mint as it is. Retiring the confirmation retires a safety mechanism: SR-15's mechanical trigger fires and a fresh reader reads this commit's delta before the re-flight (R55). **The incident (steps 19-20, 24 of the flight)** is measured in STOP 004c per R56, not accepted as this PR's defect and not dismissed: the proposals are filed with named homes (the redeem window's error surface and the auto-lock's seal-then-transition order are pre-existing product; the operator's "background retry after Close — the ladder" is his design direction for the liveness tick's lane; "a debug log file" is his direction for the diagnostics lane, shape-redacted, never a contact alias in the clear, opt-in, capped, exportable). Step 16's outcome (a locked-away invitation stays "Waiting for reply") is by design: a lock does not revoke.
- **004d (`RULING_NA0778_009` R61, the desktop half of the create/accept label-grammar asymmetry — the audit's D-7, the flight's candidate (e)):** the create field enforces the redeem field's grammar (`REDEEM_NAME_RE`, the engine's `channel_label_ok` set) and the app's shipped 32-character name cap (the redeem field itself carries none — the ruling's "redeem side's value" had no referent, so the onboarding/settings/rename value is used and said so); Activate stays disabled while the typed name is illegal, a muted hint beneath the field says "letters, numbers, - _ # only", and the typed value is never rewritten; the commit handler refuses independently. Before this, a name with a space minted an invitation every accept of which failed at the inviter's provisioning, silently, forever — the shape of the flight's stall. The engine half (invite_create validating `recipient_label` with `channel_label_ok`) is FILED (ENG, P2) for the handshake lane; the label-split lane retires the typed label as a protocol value altogether.
- **004b (`RULING_NA0778_007` R47, the delta re-read's D-01, scenario DATA only):** the redeem link's own surface-open trigger (`openRedeemEntry`, main.js :3269 at 4c7b0c76) was read by no pin and no arm; `f_u` now clears the scan's shipped marker before the redeem click and polls it after the reads, the send arm's two halves. ⚠ **D-05, for the next editor of `openInviteModal`:** `design_polish.rs`'s entering-the-mint-fresh pin reads a FIXED 400-byte window after `async function openInviteModal() {` and requires `inviteEnterMintFresh();` inside it; the R40 guard and the hide line now occupy roughly the first 190 bytes, so a longer guard or a comment INSIDE the function pushes the needle out and the pin fails to FIND the body before it fails on the property. Keep comments above the function. **R48 (the operator: AS BUILT):** the nudge stays the mockup's soft first-verification prompt; a key-changed contact's alert lives in Contacts and the detail, which now carry it; naming it in the nudge is a candidate for the contact-management lane.
- **004f (`RULING_NA0778_011` R74 as widened by `RULING_NA0778_012` R79/R80, on the two delta reads `FINDINGS_SR15_NA0778_read2_20260904T225717Z` sha256 9a034f5ba6950caf88116275f455d06cdfd663ce98665cd9d9d01a0365d6eaa6 and `FINDINGS_NA0778_R70_delta_read_20260904T230048Z` sha256 63ae6b6989e8b3d7324b8f958cb144a726816a9a2aed962206c2db4592ce76f1, banked under state/operator/NA-0778):** (a) ONE predicate `inviteLive()` (a code on screen or a mint in flight, in an open window) guards the no-op re-open AND both redeem openers (`openRedeemEntry`, `openRedeemChooser` — the page's "redeem invitation" link, the rail's "redeem", "+" and "Add contact" all reach them): while a mint is live no other opener does anything, R54's principle extended to the second window (F2-05: a stacked redeem overlay let one Escape discard a live code). (b) `inviteInFlight`, set at the Activate click after the label gate and cleared when the promise settles, refuses a second `invite_create` and, with R79, makes the three user closers no-ops during the round trip; `inviteAdoptCode(code, gen)` is GENERATION-CHECKED — a stale generation or a hidden overlay discards the code (F-01: it used to land in the hidden node); the invitation then stands on the page as "Waiting for reply" with Revoke one click away. (c) the R53 refresh-on-close moved from `closeInviteModal` (also `show()`'s structural closer — F2-07: a lock called into a sealed vault) to `inviteUserClose()`, the three gestures' closer; the f_u refresh arm is red-capable through a sentinel row planted through the renderer (F2-16). (d) `closeInviteModal` re-syncs the hint (F2-20 (a)). Residues (F-06): f_u's two stale notes corrected; the dead `.invitations-empty a.rm` rule removed; mockup 16's README row carries the supersession note (the mockup byte-identical). ⚠ **THE TESTS paragraph below describes the 004a arms and was SUPERSEDED at 004c** (the close question retired, the pins re-aimed) — it stands as written by the record's rule; the arms of record are 004c's, 004d's and this note's. **R75, NOTES NOT BYTES:** a screen transition discards a live code BY DESIGN — the lock path (shape (ii)), the keyboard's route behind the scrim (F-04), and the one unnamed MOUSE route, the native menu's File > Settings (F2-06 / E-20) — the recovery is the same for all: Revoke on the page, mint again; filed in ENG-E (the overlay/focus family). F2-15: the Received group's link toggle is NOT built — it lands with the list (ENG-B). F2-10/F2-11 beside D-05 of the 004b note: I2's guard pin and the 400-byte slice window prove PRESENCE, not position; re-aimed at 004f to `if (inviteLive()) return;` with the same window. F2-21's hairs: recorded, no act.
- **004g (`RULING_NA0778_013` R85, on the last read `FINDINGS_SR15_NA0778_read2_last_20260905T000820Z` sha256 417357f8ca8a4aa2acd339bbe1862f0490d688a0f5b591df889c80f03d0fbfa1; the LAST product change in this lane, two lines):** (a) F3-04: the mint handler's catch begins `if (gen !== inviteGen) { inviteInFlight = false; return; }` -- a stale failure paints no error into a later window and calls nothing in the vault after a lock ("distinct errors for distinct causes": an earlier mint's failure must never read as the current one's). (b) F3-06: `inviteSyncActivate()` after the stale-success return, so the next window's Activate does not wait for a keystroke. Presence pins on both (na0766); no driver arm is claimed for a rejecting mint (the harness cannot mint, `ENG-0226`) -- read-proven. **E-23's BOUND, AS MEASURED (the reader's F3-01):** the create path is bounded by reqwest 0.12.28's documented blocking default of 30 s per request (`relay_http_client()` builds the client with no explicit `.timeout(`; `qsc` Cargo.toml's `blocking` feature; Cargo.lock 0.12.28), at most two relay requests -- server-info's explicit 10 s, then the create POST -- about 40 s in all; while `inviteInFlight` is set, Activate is disabled in every window, the three gestures are no-ops and both redeem openers refuse. A command that never returns is an app-wide gateway defect, not this flag's, and NO desktop watchdog is added: a watchdog that clears the flag while the promise is pending re-opens F2-04 (a second mint) by construction. FILED for the engine: **ENG-K** (P3, the hygiene bundle beside F-16) -- an explicit `.timeout()` on the shared relay client, so the bound is the crate's, not the library's. **E-24 (F3-03):** a discarded adopt discards the DOM write only; with Activate & Copy the code is on the system clipboard, which is the user's (the kickoff's boundary, the first read's F-09). **R87, notes not bytes:** F3-02, the mirror stacking (a mint opened blind behind the REDEEM overlay by keyboard, then one Escape) is recorded with `ENG-E` (the focus-trap family) as the converse route -- no product byte; the designed path (the chooser) hides the redeem overlay first. F3-05 (a gesture inside the autolock's seal-to-transition window reaches a sealed vault, fail-quiet) is `ENG-G`'s class, recorded there. F3-07's presence-only pins and second byte window: beside D-05 of the 004b note, with F2-10/F2-11. R86: no further read -- the Director verifies the two lines from the objects; a delta beyond them draws the read.
- **004h (`RULING_NA0778_014` R91, the operator's final flight ruling on 826f3519 -- `RBANK_final_flight_rulings_NA0778_20260905`, sha256 16b4f4554e930df3df483b5f7e8b311a9948e87e709c58bd5da1f7a1b899289b; the LAST product change in this lane):** THE NUDGE IS REMOVED from the Invitations page, on both sides -- his words at 20a (3): "remove the '1 connected contact isn't..' ... I do not want that box" -- and "a little more padding above 'Sent'". Gone: the nudge element (`#invitations-nudge`), its state (`invitationsContacts`), its predicate and renderer (`invitationsRenderNudge`), its Verify-links handler, the hide in the loading state, and the contacts refresh only the predicate consumed -- the page now touches verification nowhere. Added: one spacing rule, `padding-top: var(--sp-4)` on the first group head (a token, no hex). This SUPERSEDES mockup 16's nudge line and `RULING_NA0778_004` R23's "the nudge line ships"; the audit's R-8 "verify nudge on first message" is DECLINED for this page (unverified contacts stay visibly marked in the contacts rail, which R-8 also asks). f_u: the nudge's rows removed with their landing; the node's absence asserted ONCE; the count-line arms untouched. No read (R91: a removal of a display element and one spacing rule, verified from the objects). The flight itself: PASS on every product step; the invitation flow completed both ways on the operator's own devices; `RULING_NA0778_010` R66 (4)'s explanation of the earlier legal-name stall corroborated (an awake inviter connects), not reproduced on the inviter's other machine -- the record keeps that boundary. R92 (a Verify link in the contact's detail that opens the verify window) is FILED for the contact-management lane; R93 (the consoles' `qsc_lock_nested_acquire depth=2` line, one per console) is classified at the close.

**THE TESTS.** `f_u_invitations_page` (driver: the block and its three landings, the page's empty and rendered states through the product's own renderer with synthetic records — no fixture relay, `ENG-0226` — the filters, the id-bound actions with their failure arm, the nudge and its landing, the show-once copy, the confirmation on all three gestures, and the unconditional `show()` close); `na0778_show_once_negative.rs` (the emission-site counts as exact equalities); `f_k`, `f_n`, `f_q` re-aimed (the link text; the tenth heading site); `f_d` cured. Pins re-aimed, never weakened: `na0766` I2/I7 and `na0765_both_modal_flows_have_a_visible_way_out` follow the literals the blessed changes moved, and each still fails if its property is lost.

- ⚠ **Claim boundary.** Display-only; zero `src-tauri/src/**` bytes; zero facade or invite-verb change; no `.github/**`. The overlay's own list view (`#invite-list-view`) is no longer reachable from any control and is LEFT IN PLACE with its renderer and its pins — retiring it is a small item, named here. The rows' geometry against the mockup is the operator's flight.
- **References:** `D-1421` (the spine lane); `D-0043` (the surfaces these land on); `D-0045` (its retroactive record above); `RULING_NA0778_004`; `STOP_NA0778_004`; `ENG-0226`; `ENG-0235`.

## D-0048 — NA-0779: THE DEBUG LOG's DESKTOP HALF. Settings > Diagnostics with a live, filterable, copyable, exportable ring of typed events fed by the engine's sink; the switch a stored setting; the harness captures every scenario's log; the qsc pin at the sink's merge commit.

- **Date/Lane:** 2026-09-05 · NA-0779 (spine `D-1422`) · desktop, the IMPL half of the debug-log lane · Goals G4
- **Ordered by:** the kickoff `KICKOFF_debug_log_lane_20260905.md` L3/L4/L5 and sec 5; `RULING_NA0779_002` (sha256 `8fc89fd58cd2c8da76a515048cf67e5aa7feac4210ba4bbc38dbcd6f9e45ab4c`) R2 (a)(b) and R3 on the operator's bless `RBANK_debug_log_event_model_blessed_20260905.md` (sha256 `78f10bd501d34db9ab86937eb754f8df6b3e2e68c5a838515aa6d7382e67ac14`); `RULING_NA0779_003` (sha256 `f507346eb8fb608c61fe12a936a91631817fe32d895972dd9ff38267abbd7443`) R4 (3): the pin at `#1819`'s MERGE COMMIT `4e03092fb14a6129065b711f8c17fc7252965e07`, no later bump. Layout authority: mockup 18b (`docs/mockups/mockup-18b-diagnostics-pane.html`, sha256 `9fb1940ad6db19fe65e6ec54cc558197471221e050bb889f235c7195f569f51c`, the banked bytes). Expectations sealed BEFORE the checks: `EXPECTATIONS_NA0779_004.md` (`ad393b0148e0cb1b5e59fa994243e9be3889b4560f147da329e532547a0823a6`, the pin bump) and `EXPECTATIONS_NA0779_004_part2.md` (`b4b486dc841bc5f3b0c712e1781928824179e886f4233bf19b78ee4f98068036`, the arms). The flight script, banked before the stop: `FLIGHT_NA0779_debug_log_20260905.md` (`2888d6e9ea76954d5b966c741cdf3a00dbd966a9ffe6d15787245948a0a10284`).

**WHAT SHIPPED.** (1) `src-tauri/src/debug_log.rs` (born, sha256 `a08cda30f9f248bee237fa10bd3fa5cb32e929728223917476c26571bc62f699`): THE RING (8192 events, memory only, oldest dropped and counted, one `gw.log action=drop` notice per overflow episode, cleared on lock keeping `gw.lock` with its cause as the new ring's first event, wiped by erase); the SWITCH's runtime; the DESKTOP's CLOSED VOCABULARY (`ui.surface`, `ui.tick_gate`, `ui.tick_beat`, `ui.scan_rerun`, `ui.scan_busy`, `ui.guard_refused`, `ui.autolock`; `gw.command`, `gw.lock`, `gw.unlock`, `gw.erase`, `gw.log`) as specs with typed keys and closed enum vocabularies, a constructor that REFUSES anything outside them, the `gw.command` name table pinned to `lib.rs`'s registered set by a test; the EXPORT (a header with app, version, build commit, the qsc pin, level, time, cap, buffered, dropped, seq range, a random 16-hex label from `/dev/urandom` and the format name; one line per event -- the SAME line the viewer shows, rendered by the engine's one formatter; a `# sha256=` footer over every byte before it, verified by `verify_export`), written ONCE at 0600 where the operator chose; a self-contained SHA-256 against the FIPS vectors (no new crate). (2) `settings.rs`: `debug_log = { on, level }`, omitted while default so every profile keeps its key set; the allowlist test extended. (3) `gateway.rs`: `call_named` -- ONE `gw.command` per gateway call with the command's NAME, outcome, error CODE (through `ErrorCode`: `String` errors are codes; `ErrorDto` gives its wire code, its detail never enters) and duration; the 33 gateway sites in `commands.rs` named by an instrument, not by hand. (4) `commands.rs`: `unlock_attempt` applies the stored switch and installs the engine sink on success; `lock_now` takes an optional closed `cause` (the idle timer passes `autolock`) and clears the ring keeping the cause; `erase_all` and `destroy_vault` wipe; `restart_app` names its cause; FOUR new commands -- `debug_log_read` (the viewer's poll), `debug_log_control` (on / off / level_events / level_detailed / clear, PERSISTED through the 0600 writer, the switch pushed to every screen by a Tauri event), `debug_log_event` (the front end's ONE door, `ui.*` only, refused otherwise with a `gw.log action=refused` notice), `debug_log_export` (with a directory: the file; without: the same bytes as text for Copy). (5) `lib.rs`: the module, the registration, `ui.surface` from the surface-change command. (6) `ui/index.html`, `ui/style.css`, `ui/main.js`: Settings > Diagnostics per mockup 18b (the checkbox switch, the two-way level, the counts line, the LIVE list polling every 500 ms while shown with Pause/Resume and a display-only filter, Clear, Copy, Export with a typed directory defaulting to home); the pill "LOG ON · EVENTS|DETAILED" bottom-right on EVERY screen while on; the `ui.*` sources at the tick's gate transitions, the beats, the scan's busy/rerun, the invite guards, the autolock's decisions; tokens only, no hex. (7) The qsc pin `63ece4fe` -> `4e03092f` (`src-tauri/Cargo.toml`, `Cargo.lock`). (8) THE HARNESS CAPTURE (kickoff L4): `runner.py` arms the log (on + detailed) through the app's own `debug_log_control` the first time `scr-main` is seen in a launch, exports into the run dir at every teardown, recomputes the footer digest itself, and the manifest freezes the file with its sha -- so EVERY driver scenario now carries the app's own account beside its verdict rows. (9) `f_v_debug_log_pane` (born, sha256 `a5333afa508b61e0c46080776147961d913c040a68509b20d78f81fa72f3a2af`; driver flow v) and `na0779_debug_log_arms.rs` (born, sha256 `10b79718f60a5fc8d155130f6671199b9ae7fc10ec0571efadaf6304e439732f`).

**THE ARMS, RED FIRST (STOP 002 sec 9), ALL AFTER THE SEALED EXPECTATIONS.** a1 REDACTION: RED -- the pipe shows 7 of 7 plants handed to it as a leaked event; GREEN -- the seven plants (an alias, an invite code, a bearer token, a route token, a relay URL, key material, an identity-derived hash) at EVERY desktop source: through the REAL `emit_marker` with the REAL sink, as a gateway command's ARGUMENT, and as `ui.*` fields -- 0 of 7 reach the export, no twelve-character fragment, each ui refusal leaving its notice. a2 LEVEL by list. a3 LOCK keeps only `gw.lock c.cause=...` at seq 1, the sink gone, the next unlock at seq 2; an unknown cause reads `user`. a4 ERASE wipes and switches off. a5 CAP: 20000 events -> buffered 8192, dropped 11811 (the export's own event counted), one drop notice per episode, a clear reopens the episode. a6 EXPORT: the header's fields, one line per event, the footer equal to the digest of every byte before it, one flipped byte and a truncation both fail, the file at 0600 named by time and label. a7 DISPOSAL: `recv_frame_skipped c.class=message c.disposition=left_leased` through the real choke point. a8 the vocabulary is closed (unknown name, `gw.*` from the ui door, unknown key, non-member, non-integer all refused; `reason` softens to `?`). a9 the switch round-trips and the module reads no environment variable. a10 Clear. a11 the read's contract. Plus the module's 4 unit tests (SHA-256 vectors, the pin equals the manifest's rev, the command table equals the registered set) and the `na0700` replay of the four commands through real IPC. THE SUITE at the head: 241 passed, 0 failed, 23 driver flows ignored as always, 30 binaries; `cargo fmt --check` clean; `cargo clippy --all-targets -D warnings` clean; the inventory pin reports growth only. THE GUI-DRIVER ARM `f_v_debug_log_pane` at the binary sha256 `42773323fda7fb0e79b90041a7f68d2e6b81117d06912f2eafbcd3c806d651a5`: 60 steps, PASS -- the pill "LOG ON · DETAILED" on `scr-main` before any pane opens; the pane's controls reflect the switch; the live list shows `gw.log c.action=on` / `level_detailed`, `ui.surface c.screen=scr-settings`, `gw.command c.name=protection_status`, no passphrase, no alias, every line printable ASCII; THE COPY MEASUREMENT (RULING 002 R2 (b)): `navigator.clipboard.writeText` of the export's bytes RESOLVES (the handler's result `ok:<sha>`, verdict PASS), and the read-back `navigator.clipboard.readText` is REFUSED by this WebKitGTK build (`err:NotAllowedError: The request is not allowed by the user `) -- so the Copy button ships on the write, as the ruling's (b) allows, and the read-back is recorded as refused; Pause/Resume and the filter behave; the Export button writes into the run dir with its sha; the runner's own captures at both teardowns (launch1: qsl-desktop-debug-log-20260905T202022Z-7b81f36f07372fdb.txt, launch2: qsl-desktop-debug-log-20260905T202026Z-7fc0dcaff17896b2.txt) verify their footers; after the relaunch and unlock the pill shows (the switch persisted) and the new session's first line is `seq=1 ... ev=gw.unlock` with no export line from before (the contents did not). THE CONTROL `f_c_lock_unlock` (untouched) PASSES with the capture riding it (launch1 footer_ok=True, launch2 footer_ok=True). Three export files in the arm's run dir: qsl-desktop-debug-log-20260905T202021Z-1a2bd93ed013bbb2.txt; qsl-desktop-debug-log-20260905T202022Z-7b81f36f07372fdb.txt; qsl-desktop-debug-log-20260905T202026Z-7fc0dcaff17896b2.txt.

**DEVIATIONS FROM THE BLESSED TEXT, STATED, NOT ABSORBED.** (i) FOUR commands, not STOP 002's two: the live viewer needs a read path and the switch a write path that is not `settings_set` (its two-field arity is pinned). (ii) The export's destination is a TYPED directory defaulting to home, not a save dialog: the desktop's rules forbid a new plugin, crate or capability, and the CA-file path is the tree's own precedent. (iii) `gw.command` carries `reason` from the ENGINE's closed vocabulary (`?` outside it), never the error's free text; STOP 002's `c.error_class` is that field. (iv) The harness arms the log through the app's OWN command after `scr-main` is visible, not by a pre-launch `settings.json`: that file is the S1/S2 launch-state discriminator (`state.rs`), and an environment switch is forbidden by ruling. (v) `design_polish`'s EXACT timer census moved from four to five for the pane's poll, named in the census rather than hidden behind a setTimeout chain. (vi) Three pinned needles (`na0766`, `design_round3`) are kept byte-identical with the new emits beside them, and the never-fire autolock guard moved ABOVE the surface check so it still precedes every elapsed comparison. (vii) The switch reaches the pill by PUSH (`debug-log-switch`, the menu's own carrier): the first GUI run found the front end's mirror stale after the harness flipped the switch by command. (viii) The read-back was predicted to succeed and was refused -- the seal's own boundary held. (ix) `redeem_in_flight` is in the vocabulary and is not emitted: the tree has no such flag. (x) The sink is removed at lock, so an unlock attempt's own engine markers precede the session and are not captured; `gw.unlock` opens it. (xi) The pin bump's lock diff carried four `windows-sys` edge lines beside the two git-source stanzas (cargo's re-resolve; same package set), and the desktop's OWN lock still pins `der 0.8.0` -- NOT touched (a lockfile mutation beyond the authorized pin): the desktop's advisories check is red until the Director authorizes the same one-stanza rider the spine took (`#1820`).

**THE TESTS.** `na0779_debug_log_arms.rs` (a1-a11); `debug_log::tests` (4); `na0700_ipc_replay` (+4 replays); `f_v_debug_log_pane` (driver flow v, 62 scenario steps); `settings.rs`'s allowlist case; `design_polish` census 5.

- ⚠ **Claim boundary.** One instance in Xvfb with no relay configured: the tick's beats, the relay calls, the handshake lines and a declined frame were NOT exercised by the harness (the gate reads `closed no_relay`); `ui.autolock` and the invite guards are exercised at unit level only. The flight (two devices, the production relay, an invitation) is the operator's, from the banked script. The Copy read-back is refused by the webview; the write is proven. No security-assurance claim; `README`'s posture holds. Nothing merged by the seat.
- **References:** spine `D-1422` DV-9..DV-11 (the sink, `#1819` merged `4e03092f`); `D-0040` (the tempo, the same settings pattern); `D-0046` (the notice allowlist this log stands beside); `STOP_NA0779_002` sec 3-9; `STOP_NA0779_003`; `RULING_NA0779_002`; `RULING_NA0779_003`; `ENG-0325` / G-02 (the pin).

- **004b (`RBANK_mockup_18c_blessed_20260905.md`, sha256 `27f5684bd439b96b09de796d7c73dbddb96bf25e510177e2e8e836707101341d`, on the operator's flight of STOP 004's build -- "the window looks cluttered and nothing like your mock-up"; his word on the redrawn mockup: "make sure CC follows the mock-up as closely as possible"): THE PANE REDRAWN TO MOCKUP 18c AND THE STATUS BAR ON EVERY SCREEN.** Layout authority: `docs/mockups/mockup-18c-diagnostics-proposal.html` (sha256 `4bbb70734d7c0a032c4580de76aabb114a877b053a750a3569b66dc44011f7df`, the blessed bytes, committed beside 18b; 18c governs); colour from the shipped tokens. L1 the pane fills the window (`.pane-fill`: no max-width, a flex column at the settings pane's height) and the events list is the ONLY element that grows or shrinks -- measured through the driver at the mockup's two frames, 1120x760 and 840x600: {"wide":{"log":382,"card":89,"row":28,"exp":28},"narrow_log":222}; screenshots frozen in the run dir (diagnostics_narrow_840x600.png, diagnostics_wide_1120x760.png). L2 one card: the switch labelled "Debug Log" (the tree's checkbox, drawn as the mockup's toggle), the level as a two-way segment Events | Detailed (the Invitations chips' idiom: anchors with a role, never buttons), one counts line. L3 one toolbar row (the title "Live events", the filter, Pause, Copy, Clear -- no dot, no badge) and one export row (the directory field alone with an example directory as its placeholder, then Export) with one result line; every control one height (`--sz-ctl`, measured equal across the seven controls). L4 one sentence under the title; the hints and the "How the log works" text removed. L5 THE PILL REMOVED; a STATUS BAR on the unlock screen, the chats screen and Settings, its right end "Logging: on <middle dot> level" as plain text while the log is on. L6 the bar's left end in STATE WORDS from measured signals (`RBANK_status_footer_todo_20260905.md`, sha256 `7ba7edcca5a44c216b58fd60b5ce7a936f789b858cff0bb6ff4784861f1595ed`, F2/F3): Relay not configured / configured / connected / unreachable / not trusted / unknown; Vault unlocked / locked / storage error; Auto-lock N min / off. "connected" only after a measured success (a scan that reaches the relay, a Server-pane test or probe that reports reachable; reset when the address changes); "not trusted" from a cert_not_trusted outcome; unreachable stays the accent tier. The relay's host and address are OFF the bar. NA-0752's two-source footer still feeds it (the desk's reason and the relay config); its three undrivable sentences became the three undrivable WORDS and `TICK_UNREACHABLE_COPY` became "unreachable", with their pins updated beside the text (`design_polish`, `relay_naming`, `na0764_contacts_surface`; the scenarios f_h, f_m, f_n, f_v). THE DEVIATIONS THE TREE FORCES, NAMED (L7): (a) the switch is the tree's checkbox drawn as a toggle, not a new control; (b) the pane's title keeps the tree's heading token (17px) where the mockup draws 20px -- the settings headings are pinned equal across panes (f_q); (c) the mockup's fixed 300px / 500px field widths are flex bases with a minimum, so the row never overflows at the narrow frame; (d) the narrow frame's list scrolls horizontally where the mockup clips a long line; (e) an EMPTY export field writes to the home directory's Documents if it exists, else the home directory, and the result line names where the file went -- the placeholder is the mockup's example; (f) the bar is on the three screens named (unlock, chats, Settings) and not on the wizard, erase and wiped ceremonies, which have no vault or relay state to word; (g) `.status-left` carries no ellipsis clipping -- WebKit's WebDriver reads the rendered text of an overflow-hidden span as empty, which the footer scenarios read. The bar's left text before any read: "Relay: not configured <middle dot> Vault: unlocked <middle dot> Auto-lock: 60 min" (the static default). The arms and the driver flow re-run at the binary sha256 `85152261ad3e1d64f390378abb46dcaa9b822040bd8249b717c9cab5c851cc2f`: the suite green; f_v (68 rows), f_h, f_m, f_n and the control f_c PASS. `RULING_NA0779_004` (sha256 `e9a154ef72c90a846927a91fb49e932f9ed0b1b16323d2f62543ae33a197caf2`) R6 read the footer to-do as the diagnostics lane's; the 18c bless (L6) brought the three state words INTO this commit on the operator's word, with "later" his to say at the re-flight.

- **004b, the binaries named (a correction to the binary sentence above, on measurement, the same night).** The sha256 `85152261ad3e1d64f390378abb46dcaa9b822040bd8249b717c9cab5c851cc2f` above is the `cargo build` binary measured on the 18c working tree at HEAD 346103da just before the f_v flow; the flow itself launched the binary `cargo test` builds (the bin compiled with the dev-dependencies' feature set), which was not measured then -- the sentence claimed more than was measured. Measured since: `build.rs` embeds `git rev-parse HEAD`, so every commit changes the binary's bytes; and at one head `cargo test`'s bin and `cargo build`'s differ. At the pushed head 0ec18f06 (the layout commit 5fbeed2b and the two whitespace-only restorations of f_m's and f_n's serialization, a32bd5ac and 0ec18f06) the test-configured binary is `67cb3b04b9ef529dec04ab833cb3ab75004dbe42c100062222d047ddfbbe1ade`, the same before and after the five driver flows re-run there, all PASS (f_v 68 rows, f_h 30, f_m 47, f_n 93, the control f_c 32; the run dir `gui_runs_18c_final_20260906T012500Z`, whose first attempt was killed by the host's low-memory guard during f_v and left as it is); `cargo build` there gives `a5c758c0148bf624a8a0bfd1b9a4d008ff67ab0b0975a0b46a9ce10559348d5f` three times (twice as-is, once after touching main.rs). THE FLIGHT BINARY for the operator's re-flight of steps 2-5 and 9-15 is `cargo build` at the head this paragraph lands on, named with its path in the banked `FLIGHT_BUILD_<head>.txt` -- a head cannot name its own sha. The board at 0ec18f06: advisories, gui-driver, infra-literal-scan, rust and the advisory stable-canary all pass; the PR CLEAN.

- **004c (`RBANK_second_flight_18d_blessed_20260905.md`, sha256 `0cc78bee20eb6f2e3ad399a55f49fdbba99e71e56e0b61635d49ff9ba9e8cfe7`; the operator's second flight, of the 18c build; his eleven findings I1-I11 blessed on mockup 18d, `docs/mockups/mockup-18d-diagnostics-proposal.html`, sha256 `8a816162c1bbd3ac2d7484061cb6aaec2cdc2b07b7dd67e54457a2a4e78b9472`, the LAYOUT authority for the pane and the bar; the rail's reference `docs/mockups/mockup-rail-icons-preview.html`, sha256 `5ed12fe26f8367c8f4fd46c87e758af4205c4766a553396a6aa7ae6e796e493d`, from `RBANK_rail_icons_todo_20260905.md`, sha256 `6013a32c7a3ffab6e3b9b0171e64fa743e386d91ee2f1dc610fbabb89523e21b`, promoted into this PR by the operator's "now"; the shipped tokens the colour authority): ONE FURTHER COMMIT CARRYING ALL ELEVEN.** I1 THE BAR ON THE MAIN WINDOW ONLY: every screen inside it (chats, contacts, settings) keeps its bar; the unlock window's footer is removed (its two texts overlapped at that width) and no other window carries one -- the driver asserts no bar on scr-unlock and exactly two footers (scr-main, scr-settings). I2 `#contacts-rows` gains a top padding (`--sp-3`, 12px, measured) above the first contact. I3 the sidebar item and the pane title read "Security". I4 THE MISS AGAINST THE SEALED FLIGHT STEP 13 ("the counts line keeps growing on the next beat"): the 18c build paused the whole refresh; now the poll ALWAYS reads (the counts line keeps growing, the new lines are buffered) and only the RENDER waits -- the list shows nothing newer than the moment of the pause whatever re-renders it (the filter), and Resume shows everything that arrived. The driver's Pause arm is extended: two gateway calls are provoked while Paused; measured {"counts":16,"shown":14} at the pause, then the counts rose while the shown lines stayed, then the list caught up on Resume. I5 THE EXPORT LINE, as blessed: "Saved <middle dot> the name's tail from its random label <middle dot> N KB" -- no path, no sha; cleared after ten seconds (one one-shot timer, the only new timer; never a poll) or when the pane is left; a failure line stays until the pane is left. The driver reads the shape and polls past the ten seconds for the clear. DEVIATION NAMED: the RBANK says the full path "stays in the log's own gw.log c.action=export event" -- it does NOT and cannot: the desktop vocabulary carries no free text by its own rule (D-0048, redaction by type), so the export event marks the fact and never the path; the directory is the field's, or the placeholder's own default (Documents, else home) when the field is empty. I6 THE RAIL: the three tabs as one family of outline icons drawn by the Director (T2's shapes verbatim, stroke 1.6, no fill; the sliders' knobs filled with the surface they sit on), in both rails (the main window's and Settings'); the reference's size (a 28px icon in a 36px square in the 52px rail) scoped to `.rail` so the contacts header's "+" keeps its pinned 34px / 21px shape; ONE selection treatment -- a soft filled square in the raised-surface token (`--bg-raised`), the icon in the text colour, no border (the previous active treatment, the accent-tinted `--rail-active-bg` with the accent-coloured icon, retired for the rail; the token stays for the selected contact row). I7 the bar's right end reads "Debug Log: events" or "Debug Log: detailed" while the log is on and nothing while off. I8 the "Level" label before the segment removed (the radiogroup keeps its aria-label). I9 the "N accepted" after Sent removed from the Invitations count line (the Accepted chip still lists them; f_u's two expectations re-pinned). I10 Export in the neutral (secondary) tier. I11 the new contact's row shows the blue link "verify" alone -- the shipped `a.rm.plain` text link carrying the row's word class, role button, focusable; it opens the row as the row itself does (f_n re-pinned; measured tag A, classes `rm plain contact-word is-accent`). NOT CHANGED, NAMED: the detail pane's own "New contact -- verify identity" line above the callout (not among the eleven). THE MEASUREMENTS on the 18d tree (HEAD 8de9a984 with these changes uncommitted; the test-configured binary the flows launched `1703e0f58dfaca128cfefbee58078317028a6000ea549ab5b45bd76ee00bfa0c`, the same before and after): the suite 241 passed / 0 failed / 23 ignored; fmt and clippy clean; the driver flows all PASS -- f_v 75 rows (the two frames, the Pause arm, the export line and its clear, no bar on unlock, a frozen picture of the main window), f_h 30, f_n 95 (a frozen picture of Contacts with the verify link), f_u 163, f_m 47, the control f_c 32. The operator re-flies steps 2-5 and 9-15 and looks at Contacts, Invitations and the unlock window; the read stays commissioned after that flight, over the whole of #56.

- **004c, the rail square (a correction on CI's measurement, the same night).** At c58916a5 CI's gui-driver job was RED on ONE flow, f_q (na0776_gui_q_heading_alignment): the pane headings are pinned to the centre of the rail's first button, and I6's 36px button (the reference's square) moved that centre by 1px (measured by the flow: centre-vs-rail=1.00 for chats_list and contacts_list; the seat had run six flows locally, not all twenty-two). The rail button stays the tree's 34px (`--sz-head-row`, the shared head-row height) with the 28px icon inside it -- the square is 34px where the reference draws 36px, a deviation the tree forces, named in style.css beside the rule. All 22 driver flows PASS locally on the corrected tree (the test-configured binary `b29e9cd2473e6c90a0bd3fb3354ba766cfa76e8de5424220ee0d7afa1e988ad6`, the same before and after; the run dir `gui_runs_18d_all_20260906T034837Z`); the suite 241 passed / 0 failed; fmt and clippy clean. The lesson is recorded: the driver suite is run WHOLE before a push, not the flows the change names.

- **004c, I12 -- ONE NAME PER PERSON on the Sent list (the operator's second-flight finding: renaming a contact did not change that person's name in Invitations > Sent for an accepted invitation; the Director's position, banked at the close: once an invitation has become a contact, the Sent list displays the contact's CURRENT name; the mint-time label stays in the record unrewritten; pending invitations keep their label).** MEASURED FIRST, as ordered -- does an accepted invitation record reference the contact it became? The engine's `InviteRecord` (qsc `invite/mod.rs` at the pin 4e03092f) carries `invite_id` (its own slot's route token, not the peer's), `cap`, `expiry`, `relay_ep`, `state` (unit variants; `Redeemed` carries nothing), `revoke_token`, `created_unix`, `label`; the facade's `InviteSummary` and the desktop's `InviteDto` expose `invite_id`, `state`, `expiry`, `revocable`, `label`, `created`. The contact side, `ContactSummary` / `ContactDto`: `alias`, `fingerprint`, `pinned`, `blocked`, `state`, `display_name`, `device_count` -- no invite id, no origin. No stored id, no peer route token, no hash on either record. THE REFERENCE IS THE LABEL: the desktop's ONLY accept path (`autoConnectClass`, NA-0764 RULING R1) provisions the contact under `alias := the invite's own label` and never accepts an unlabelled invitation, and the alias is immutable -- a rename writes `display_name` beside it (NA-0765 A3). So an accepted invitation names its contact by `contact.alias === invite.label`, exact, by the tree's own construction rule; that is the field, and it is a reference the tree already relies on, not one invented here. THE DISPLAY-SIDE LOOKUP: `refreshInvitationsPane` reads `contact_list` beside `invite_list` on every open of the pane (refresh-on-open, the invite module's own decision; a refused read leaves the labels to speak) into `invitationsContacts`; `invitationsRowEl` names an ACCEPTED row by the matching contact's current name (`contactDisplayName`: `display_name`, else the alias, which IS the label) and every other row by its label; nothing is written. CAVEATS NAMED: two accepted invitations minted under the same label would name the same contact (the second accept under an existing alias is the engine's to refuse, not this lookup's); a contact removed after acceptance leaves the row on its label; a contact accepted by any other path (the CLI, with an alias of its own choosing) is not matched -- the rule is the desktop's. THE ARM (f_u, on fixture rows): i2 accepted/'dana' with contact dana 'Dana R.' reads 'Dana R.'; i1 PENDING/'Marcus' keeps 'Marcus' although a contact 'Marcus' exists; i3 accepted/'Priya' with no exact-alias contact keeps 'Priya'; the rename to 'Dana Rivera' moves i2 alone; a cleared name reads 'dana'; a gone contact falls back to 'dana'. All 22 driver flows PASS on this tree (the test-configured binary `039e3838e8b30042941130e478c64c3c7decd32467083168ff49ffad5828bcf3`, the same before and after; run dir `gui_runs_18d_i12_20260906T050141Z`); the suite 241 passed / 0 failed; fmt and clippy clean.

- **004d (`RULING_NA0779_005_20260906.md`, sha256 `8e1e9d804498e9eb045443612be1e1f51df05bb9608acdbb12feb9ed142b352f`, R2, ruled from the SR-15 cold read `FINDINGS_SR15_NA0779_20260906T074720Z.md`, sha256 `80247aed9de73cad729b1f10df37d044f0087e91043a43a3d5946086cc53dacd`, STOP 0 / FIX 4 / NOTE 15): ONE FURTHER COMMIT, RED ARMS FIRST, F-04 FIRST.** THE RED RUN (banked `ARMS_R005_red.txt`): five unit arms a12-a16 written against the behaviour BEFORE any fix so they compile and fail on their assertions (a12 out=ok for a rejected passphrase; a13 five events in the locked ring; a14 the sink installed with no session; a15 the zero label; a16 its own second half), two design pins (N-17, N-18) red, the driver arms in f_v (two Copies' labels) and f_m (the probe's out) red. THE FIXES: F-04 -- `CommandOutcome` for `Result<T,E>` now reads `Ok(t)` as `t`'s OWN outcome; `UnlockDto` (Rejected -> passphrase_rejected, Delayed -> unlock_delayed, Wiped -> vault_wiped, VersionUnsupported -> vault_version_unsupported) and `RelayTestDto` (Unreachable -> relay_unreachable, CertNotTrusted -> relay_cert_not_trusted, and NotAQslRelay -> relay_not_a_qsl_relay, the seat's addition of the same class, named; AuthRequired stays ok: the relay was reached) say fail with a closed reason; `Finished(bool)` wraps invite_finish (false -> not_finished); the DTO is built INSIDE the named call for the two probes so the gateway reads it; `DESKTOP_REASONS` (eight members) joined to the reason lookup after the engine's vocabulary; `gw.unlock out=fail` EMITTED from the Rejected and Delayed arms (`unlock_failed`). F-02 -- the intake is keyed on the session: `push` refuses while `unlocked_at` is None and COUNTS NOTHING (the count of attempts is itself kept out); `gw.lock` enters through its own door; `on_unlock` reopens. F-03 -- `apply_action("on")` installs the sink only while a session is open. F-01 + N-05 -- `export_snapshot(build, act)` pushes the act's own word (`gw.log action=export` for the file, `action=copy` for the clipboard, a new member), mints a label from the OS RNG for BOTH arms, then renders through a now side-effect-free `export_text`; the Text DTO carries the label; the Copy handler records it. N-07 -- `relayProven` and `relayTrust` reset in `showUnlockScreen`, the one function every lock path reaches. N-17 -- the off_surface arm carries the decision alone. N-18 -- the failure line's class is `is-trouble` in the accent colour. N-14 -- the sentence at `install_sink` (the desktop's half: the engine calls the sink under its sink mutex; a sink must not emit a marker). N-16 -- a16. FOUND BY THE ARMS, FIXED WITH THEM: `vault_create` unlocks the vault but never told the log a session opened; the `on`-only intake had let the wizard's session in, and the harness's first launch goes through it -- it now calls `on_unlock` as `unlock_attempt` does. DEVIATIONS NAMED: (a) F-04's unlock arms are UNIT arms on the outcome mapping and the emitter, because F-02 closes the intake while locked -- in the app a failed passphrase is never in any ring, by F-02's own design; the driver's F-02 arm proves the app path (lock, a surface change, a wrong passphrase, the right one: seq 1 gw.lock, seq 2 gw.unlock, no unlock_attempt anywhere); (b) a `ui.surface c.screen=scr-unlock` AFTER gw.unlock is the page's height sync of the surface it leaves, fired once the intake is open, and is not forbidden; (c) the tick probes no relay while there are no contacts, so f_m's F-04 arm provokes `relay_probe` itself; (d) the engine-side sentence in `set_event_sink`'s doc (N-14) rides the spine's records PR, not #56; (e) the two `gw.command` doors differ by design on an unlisted name (the live path substitutes `?`, the spec door refuses); (f) `Option<T>` and `Vec<T>` answers stay `ok` (an accept that consumed nothing this poll is not a failure). THE MEASUREMENTS on the tree with these changes uncommitted (HEAD bb46ed84): the suite 248 passed / 0 failed / 23 ignored (seven new tests, the inventory re-pinned at 271 with `--write`, which also pins the eleven a1-a11 arms that were never pinned); fmt and clippy clean; the driver suite of 22 on the test-configured binary `c8cad75e556d040e3782cf0c64e52bc99894713f3c763ea547f8737b8bf498d7` (the same before and after): 14 of 22 PASS on every arm row. 8 flow(s) carried an isolation-bracket FAIL with EVERY arm row PASS: the bracket's real-HOME census diff names only na0701_gui_a_read_census: ~/.local/share/org.quantumshieldlabs.qsldesktop/qsc, ~/.local/share/org.quantumshieldlabs.qsldesktop/qsc/vault.qsv; na0701_gui_b_onboarding: ~/.local/share/org.quantumshieldlabs.qsldesktop/qsc, ~/.local/share/org.quantumshieldlabs.qsldesktop/qsc/qsp_sessions, ~/.local/share/org.quantumshieldlabs.qsldesktop/qsc/qsp_sessions/Alice1.qsv, ~/.local/share/org.quantumshieldlabs.qsldesktop/qsc/vault.qsv; na0701_gui_c_lock_unlock: ~/.local/share/org.quantumshieldlabs.qsldesktop/qsc, ~/.local/share/org.quantumshieldlabs.qsldesktop/qsc/qsp_sessions, ~/.local/share/org.quantumshieldlabs.qsldesktop/qsc/qsp_sessions/Alice2.qsv, ~/.local/share/org.quantumshieldlabs.qsldesktop/qsc/vault.qsv; na0701_gui_d_settings_persistence: ~/.local/share/org.quantumshieldlabs.qsldesktop/qsc, ~/.local/share/org.quantumshieldlabs.qsldesktop/qsc/vault.qsv; na0701_gui_e_erase_ceremony: ~/.local/share/org.quantumshieldlabs.qsldesktop/qsc, ~/.local/share/org.quantumshieldlabs.qsldesktop/qsc/qsp_sessions, ~/.local/share/org.quantumshieldlabs.qsldesktop/qsc/qsp_sessions/Alice4.qsv, ~/.local/share/org.quantumshieldlabs.qsldesktop/qsc/qsp_sessions/Alice5.qsv, ~/.local/share/org.quantumshieldlabs.qsldesktop/qsc/qsp_sessions/Bob3.qsv, ~/.local/share/org.quantumshieldlabs.qsldesktop/qsc/vault.qsv; na0701_gui_f_menu_event_substitutes: ~/.local/share/org.quantumshieldlabs.qsldesktop/qsc, ~/.local/share/org.quantumshieldlabs.qsldesktop/qsc/vault.qsv; na0750_gui_g_fingerprint_two_tier: ~/.local/share/org.quantumshieldlabs.qsldesktop/qsc, ~/.local/share/org.quantumshieldlabs.qsldesktop/qsc/qsp_sessions, ~/.local/share/org.quantumshieldlabs.qsldesktop/qsc/qsp_sessions/Alice6.qsv, ~/.local/share/org.quantumshieldlabs.qsldesktop/qsc/vault.qsv; na0754_gui_j_relay_pane_truth: ~/.local/share/gnome-shell -- the operator's concurrent desktop use (a live app instance from ~/work/flights and the desktop session's own state), outside the harness's profile, which runs the app under the run's XDG directories; named, not counted as the arms' failure.

- **004e (THE DIRECTOR's RETURN on the acceptance export of 962f01ab, 2026-09-06, before the merge; the exports themselves verified: files 41debfef... sha 746cc4e5... and 7ba3bd32... sha 402d74f6..., digests EQUAL, 0 hits on every needle, the exchange in 35 s).** Bob's seq 59 read `gw.command out=fail reason=not_finished c.name=invite_finish` right after his handshake COMPLETED as responder (seq 55 handshake_complete, 57 invite_finish_hs_offer, 58 producer_ack): the engine's bool answers "did MY redeem finish" (`Ok(true)` only on the redeemer's selected path -- the page has said so since NA-0768, `invite/mod.rs:1649`), so F-04's `false -> not_finished` called the inviter's completed A2 a failure -- the exact class F-04 fixed for unlock. THE FIX, ONE COMMIT: `invite_finish` returns a TYPED result, `InviteFinish::{Finished, Offered, Nothing}`, classified inside the named call from the bool and the NAMES of the markers the engine emitted during that very call (`invite_finish_hs_offer` -> offered; peeked from the marker queue before the gateway drains it; names only, a closed vocabulary, never a value); every answer is `out=ok` with `c.result=<member>` (`RESULT_WORDS`: finished | offered | nothing, a new enum key on gw.command's spec; an unlisted word is omitted, never copied); `out=fail` is an actual error only; `not_finished` is retired from `DESKTOP_REASONS`; the command returns the word to the page, which compares it by equality (`done === "finished"`; the pin moved with it). `CommandOutcome` gains `result_word()` (None for every other type); `gw_command` carries the result. THE ARMS, RED FIRST: a17 measured red against the bool newtype as it stood (`ARMS_R005_return_red.txt`: Fail/not_finished where Ok was expected; the spec refusing `result`), then rewritten to the typed result at the green run -- the three classifications, every answer ok with its word, an actual error the only fail with no result word, the live line `out=ok dur=3 c.name=invite_finish c.result=offered` equal to the spec door's, free text refused and omitted. THE DRIVER ARM the return asked for -- the inviter's A2 finish reading `out=ok c.result=offered`, an idle scan reading `out=ok c.result=nothing` -- NEEDS A RELAY THAT ANSWERS; this harness has none, so the drivable half is asserted in f_m (a finish against the relay that never answers is an ACTUAL error: `out=fail` with the engine's reason, never `not_finished`, no `c.result` on a failed line; it was green from the day it was written, a guard), the mapping is pinned by a17, and the operator's export on this head is where the two words show live -- NAMED as the deviation. THE MEASUREMENTS on the tree with these changes uncommitted (HEAD 962f01ab): the suite 249 passed / 0 failed / 23 ignored (a17 new; the inventory re-pinned at 272); fmt and clippy clean; the driver suite of 22 on the test-configured binary `97c5f5f01001bac376a1d0ca0b38f334220975735df5542877c75ee5972105e9` (the same before and after): 17 of 22 PASS. 5 flow(s) carried an isolation-bracket FAIL with every arm row PASS; the census diff names only na0701_gui_c_lock_unlock: ~/.cache, ~/.cache/opencode, ~/.config, ~/.config/opencode, ~/.local/share, ~/.local/share/opencode, ~/.local/state, ~/.local/state/opencode; na0701_gui_e_erase_ceremony: ~/.local/share/gnome-shell; na0750_gui_g_fingerprint_two_tier: ~/.local/share/opencode; na0753_gui_i_flight_fixes: ~/.local/share, ~/.local/share/opentui, ~/.local/state/opencode; na0756_gui_l_invite_redeem: ~/.local/state/opencode -- external to the harness's profile, as in 004d.
